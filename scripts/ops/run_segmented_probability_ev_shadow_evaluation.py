#!/usr/bin/env python3
"""Run EV/risk-adjusted shadow evaluation for segmented probability candidates."""
...
from research.signal_evaluation.segmented_probability_ev_shadow_evaluation import (  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate segmented probability shadow routes by realized EV/risk usefulness. "
    ...
    parser.add_argument("--min-risk-adjusted-improvement-bps", type=float, default=2.0)
    ...
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_SEGMENTED_PROBABILITY_EV_SHADOW_DIR)
...
def main() -> int:
    ...
    artifact = write_segmented_probability_ev_shadow_evaluation_report_from_path(
    ...
    payload["recommended_policy_risk_adjusted_return_delta_bps"] = selection.get(
        "recommended_policy_risk_adjusted_return_delta_bps"
    )
    payload["recommended_policy_hit_rate_delta"] = selection.get("recommended_policy_hit_rate_delta")
